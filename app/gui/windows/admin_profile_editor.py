"""
Admin Profile Editor for editing other users' profiles.

This module provides an interface for administrators to edit any user's
profile information.
"""
...
class AdminProfileEditor:
    """
    Admin profile editor window for editing other users' profiles.
    
    Allows administrators to modify any user's name, email, and phone number
    ...
    def __init__(
        ...
        """
        Initialize the admin profile editor.
        
        Args:
            user_data: User data to edit
        ...
        self.window.title(f"Редактирование профиля - {user_data.get('name', 'Пользователь')}")
    ...
    def _setup_ui(self) -> None:
        ...
        title_label = ttk.Label(
            main_frame,
            text=f"Редактирование профиля пользователя",
            font=("Arial", 14, "bold")
        )
        ...
        self.name_entry.grid(row=3, column=0, columnspan=2, sticky=(tk.W, tk.E), pady=(0, 10))
        ...
        self.email_entry.grid(row=5, column=0, columnspan=2, sticky=(tk.W, tk.E), pady=(0, 10))
        ...
        self.phone_entry.grid(row=7, column=0, columnspan=2, sticky=(tk.W, tk.E), pady=(0, 20))
        
        # Buttons frame
        buttons_frame = ttk.Frame(main_frame)
        buttons_frame.grid(row=8, column=0, columnspan=2, pady=(0, 10))
    ...
    def _center_window(self) -> None:
        ...
        center_window(self.window, 400, 400)
    ...
    def _load_user_data(self) -> None:
        ...
        set_entry_text(self.name_entry, self.user_data.get('name', ''))
        set_entry_text(self.email_entry, self.user_data.get('email', ''))
        set_entry_text(self.phone_entry, self.user_data.get('phone', ''))
    ...
    def _perform_save(self, form_data: Dict[str, str]) -> None:
        ...
        """
        try:
            # For admin editing other users, we need to use a different approach
            # Since the current API doesn't have admin-specific user update endpoint,
            # we'll simulate it by temporarily updating the profile
            # In a real application, you'd have an admin-specific endpoint
    ...
    def _on_save_complete(self, success: bool, error: str) -> None:
        ...
        if success:
            ...
            show_success_message("Успех", "Профиль пользователя успешно обновлен!")
    ...
    def _close_window(self) -> None:
        """Close the admin profile editor window."""
        try:
            self.window.destroy()
        except tk.TclError:
    ...
    def destroy(self) -> None:
        """Destroy the admin profile editor window."""
        try:
            self.window.destroy()
        except tk.TclError:
            pass  # Window already destroyed
    ...
    def is_visible(self) -> bool:
        """
        Check if the admin profile editor window is visible.
    ...
    def show(self) -> None:
        """Show the admin profile editor window."""
    ...
    def hide(self) -> None:
        """Hide the admin profile editor window."""
